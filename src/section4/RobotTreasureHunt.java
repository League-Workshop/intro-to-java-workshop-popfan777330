...
public class RobotTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = Arnold.getX();
		int robotYLocation = Arnold.getY();

		// 6. Print the robotXLocation and robotYLocation variables to the console
		System.out.println(robotXLocation + " and " + robotYLocation);
		// 7. If robot is at same location as the little girl
		if (robotXLocation == 720 && robotYLocation == 423) {
			// --make a pop-up tell the robot where to go next
			JOptionPane.showMessageDialog(null, "Now go to the skulls' eye!");
		}
		...
		if (robotXLocation == 204 && robotYLocation == 57) {
			treasureFound();
		}
	}
	...
	private void go() {
		...
		// JOptionPane.showMessageDialog(null, "Ask the girl for help with your quest.
		// Press the space bar to ask.");
	...
	static void treasureFound() {
		try {
			URI uri = new URI("https://www.youtube.com/watch?v=jgn00qHdEOg");
